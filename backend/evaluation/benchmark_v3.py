...
def dense_similarity(distances):
    return np.array([1.0 / (1.0 + float(distance)) for distance in distances])
...
def get_bm25(query):
    documents, scores, ids, metadatas = bm25_search(
        query=query,
        top_k=TOP_K
    )
    return documents, scores, ids, metadatas
...
def get_dense(query):
    documents, distances, ids, metadatas = dense_search(
        query=query,
        top_k=TOP_K
    )
...
def get_adaptive_v3(query):
    # 1. Retrieve once
    dense_docs, dense_distances, dense_ids, dense_metadatas = get_dense(query)
    bm25_docs, bm25_scores, bm25_ids, bm25_metadatas = get_bm25(query)

    # 2. Extract retrieval features
    from services.retrieval_feature_service import retrieval_feature_service

    retrieval_features = retrieval_feature_service.extract(
...
def get_rrf(query):
    dense_docs, _, _, _ = get_dense(query)
    bm25_docs, _, _, _ = get_bm25(query)
...
def evaluate_results(results, source_document):
    retrieved_documents = [
        item["document"] if isinstance(item, dict) else item
        for item in results
    ]
    ...
    recall = 1.0 if source_document in retrieved_documents else 0.0
...
def main():
    ...
    for index, row in test_df.iterrows():
        ...
        if (
            ...
            print(f"Processed {index + 1}/{len(test_df)}")
        ...
        result = evaluate_results(
            [{"document": doc} for doc in bm25_docs],
            source_document
        )
        ...
        result = evaluate_results(
            [{"document": doc} for doc in dense_docs],
            source_document
        )
        ...
        # Existing Adaptive
        start = time.perf_counter()
        existing_results = get_existing_adaptive(query)
        latency = time.perf_counter() - start
        ...
        # V3 Adaptive
        start = time.perf_counter()
        v3_results = get_adaptive_v3(query)
        latency = time.perf_counter() - start
    ...
    output_path = "evaluation/v3_retrieval_results.csv"
