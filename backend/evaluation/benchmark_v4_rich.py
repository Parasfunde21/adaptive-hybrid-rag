...
def build_rich_features(query, bm25_docs, bm25_scores, dense_docs, dense_distances):
    ...
    if len(dense_similarity):
        ...
        dense_gap = (
            dense_top - float(dense_similarity[1])
            if len(dense_similarity) > 1
            else dense_top
        )
    ...
    avg_length = (
        sum(len(token) for token in tokens) / query_count
        if query_count
        else 0.0
    )
...
def get_v4_rich(query, model, prediction_map):
    (
        ...
        dense_ids,
        dense_metadatas
    ) = dense_search(query=query, top_k=TOP_K)
    ...
    (
        ...
        bm25_ids,
        bm25_metadatas
    ) = bm25_search(query=query, top_k=TOP_K)
...
def main():
    ...
    evaluation_df = test_df[
        test_df["query"].astype(str).isin(test_queries)
    ].copy()
    ...
    for index, row in evaluation_df.iterrows():
        ...
        print(f"Processed {index + 1}/{len(evaluation_df)}")
        ...
        # V4 Rich Adaptive
        start = time.perf_counter()
        v4_results, v4_weight = get_v4_rich(
            query=query,
            model=model,
